    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                z.append(y.view(bs, -1, self.no))
                logits_.append(logits.view(bs, -1, self.no - 5))  # 修改---3
        return x if self.training else (torch.cat(z, 1),) if self.export else (torch.cat(z, 1), torch.cat(logits_, 1), x)  # 修改---4
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, SPPFCSPC, SPPCSPC, SPPCSPC_group, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C2f, C3, C3TR, C3STR, C3SPP, C3Ghost, SELayer, GAM_Attention, CBAM,  ECA, eca_layer, DecoupledHead]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, help='model.yaml')
